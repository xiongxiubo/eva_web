export class Lipsync {
  ...
  constructor() {
    ...
    Object.keys(this.rules).forEach((key: string) => {
      this.rules[key] = this.rules[key].map((rule: string) => {
  ...
  convert_digit_by_digit(num: any) {
    ...
    let numWords = "";
    for (let m = 0; m < num.length; m++) numWords += this.digits[num[m]] + " ";
    return numWords.substring(0, numWords.length - 1);
  }
  ...
  convert_millions(num: number): string {
    if (num >= 1000000) return this.convert_millions(Math.floor(num / 1000000)) + " million " + this.convert_thousands(num % 1000000);
    return this.convert_thousands(num);
  }
  convert_thousands(num: number): string {
    if (num >= 1000) return this.convert_hundreds(Math.floor(num / 1000)) + " thousand " + this.convert_hundreds(num % 1000);
    return this.convert_hundreds(num);
  }
  convert_hundreds(num: number): string {
    if (num > 99) return this.ones[Math.floor(num / 100)] + " hundred " + this.convert_tens(num % 100);
    return this.convert_tens(num);
  }
  convert_tens(num: any): string {
    if (num < 10) return (Number(num) != 0 && num.toString().startsWith("0") ? "oh " : "") + this.ones[Number(num)];
    if (num >= 10 && num < 20) return this.teens[num - 10];
    return this.tens[Math.floor(num / 10)] + " " + this.ones[num % 10];
  }
  convertNumberToWords(num: any): string {
    if (num == "0") return "zero";
    if (num.startsWith("0")) return this.convert_digit_by_digit(num);
    if ((num < 1000 && num > 99) || (num > 10000 && num < 1000000)) return this.convert_digit_by_digit(num);
    if ((num > 1000 && num < 2000) || (num > 2009 && num < 3000)) return num % 100 != 0 ? this.convert_sets_of_two(num) : this.convert_tens(num.substring(0, 2)) + " hundred";
    ...
    while (o.i < chars.length) {
      ...
      if (ruleset) {
        for (let i = 0; i < ruleset.length; i++) {
          ...
          if (matches) {
            rule.visemes.forEach((viseme: string | number) => {
